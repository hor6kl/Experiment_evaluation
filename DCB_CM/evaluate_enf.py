...
class Evaluate_ENF:
    ...
    def return_voltage_indexes(self) -> []:
        ...
        num_iter_r_v = common_functions.found_indexes_Voltage(self.voltage)

        # indexes are striped to evaluated values from photos
        num_iter_r_v = num_iter_r_v[
    ...
    def compute_compliance_ENF(self, path: str, num_iter: [int]) -> float:
        ...
        # c0 + c1*x + c2*x**2
        c0, c1 = np.polynomial.polynomial.Polynomial.fit(a, C, 1, domain=[])

        delta = -c0 / c1

        # expanding the polyfit curve to be visible in plot
        x_axis = np.linspace(min(a), max(a), 200)
        ...
        plt.xlabel("$a^3$ [N/m]", fontsize=12)
        plt.ylabel("$C$ [m]", fontsize=12)
    ...
    def create_R_cuve_ENF(self, time_delay_R, path: str):
...
def graph_ENF_R_curves(
    ...
    plt.ylabel(r"Fracture toughness $G_{II}$ [J$m^{-2}$]")

    plt.legend(name_TAR, fontsize="7")
    plt.grid()
