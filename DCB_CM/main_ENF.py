...
for counter, specimen in enumerate(spec_data):
    if specimen.data_value[1] == 1 or specimen.data_value[1] == str(1):
        ...
        # initiaze class
        evaluate_ENF = Evaluate_ENF(Force_i, deltaY_i, time_i, delam_t, delam_R, specimen, names_param, name_TAR_str)

        ENF_G_II, delam_set = evaluate_ENF.create_R_cuve_ENF(time_delay_R, path_fig_check, voltage_i, strip_range)
...
graph_ENF_R_curves(ENF_G_II_set, delam_set_array, name_TAR, color_set, marker_set, line_set, path_fig_R_ENF, range_min)
